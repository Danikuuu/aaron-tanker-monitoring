...
<body class="overflow-hidden" x-data="{ sidebarOpen: false }">
    ...
    <div class="flex min-h-screen">
        ...
        >
            <div class="flex flex-col flex-1 min-h-0">
                ...
                <nav class="px-3 xl:px-4 space-y-1 overflow-y-auto flex-1 pb-2">
                    ...
                    <a href="{{ route('super_admin.audit-logs') }}"
                       class="flex items-center gap-3 px-4 xl:px-6 py-3 xl:py-4 rounded-lg hover:bg-white hover:text-primary transition
                              {{ request()->routeIs('super_admin.audit-logs') ? 'bg-white text-black' : '' }}">
                        <svg class="w-5 h-5 xl:w-6 xl:h-6 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                  d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
                        </svg>
                        <span class="text-sm xl:text-md">Audit Logs</span>
                    </a>
                </nav>
            </div>
                ...
                <div class="flex items-center gap-3 sm:gap-4 shrink-0">
                    ...
                    <div class="flex items-center gap-2 sm:gap-3 bg-primary py-1 px-2 sm:px-3 rounded-l-3xl">
                        ...
                        <div class="hidden sm:block">
                            <div class="font-semibold text-white text-sm">{{ trim((auth()->user()->first_name ?? '') . ' ' . (auth()->user()->last_name ?? '')) ?: 'Admin' }}</div>
    ...
    (() => {
        ...
        function fetchNotifications() {
            fetch('{{ route("super_admin.notifications") }}', { headers: { 'Accept': 'application/json' } })
                .then(r => r.json())
                .then(data => {
                ...
                modalBody.innerHTML = `
                    ...
                    <div class="modal-meta-grid">
                        <div class="modal-meta-item">
                            ...
                            <p style="font-weight:700;">${n.name}</p>
                        </div>
                        <div class="modal-meta-item">
                            <label>Email</label>
                            <p>${n.email}</p>
                        ...
                        <div class="modal-meta-item">
                            <label>Status</label>
                            <p style="color:${n.status === 'approved' ? '#276749' : '#C53030'};font-weight:700;">${ucfirst(n.status)}</p>
                        </div>
