...
@section('title')Payment Details — Nº {{ $receipt->receipt_no }}@endsection
    ...
    <div class="flex-1 space-y-4">
        ...
        <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-5">
            ...
            <div class="relative pl-6">
                ...
                <div class="relative">
                    <div class="absolute -left-4 top-1 w-3 h-3 rounded-full {{ $isOverdue ? 'bg-red-500' : 'bg-gray-300' }} border-2 border-white shadow"></div>
                    <div class="border {{ $isOverdue ? 'border-red-200 bg-red-50' : 'border-gray-200 bg-gray-50' }} rounded-lg p-3">
                        <div class="flex items-center justify-between">
                            <div>
                                <p class="text-xs font-semibold {{ $isOverdue ? 'text-red-600' : 'text-gray-500' }} uppercase tracking-wide">
                                    Final Payment Due
                                </p>
                                <p class="text-sm font-bold {{ $isOverdue ? 'text-red-700' : 'text-gray-700' }}">
    ...
    <div class="w-full xl:w-[360px] shrink-0">
        <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-5 sticky top-6">
            ...
            <div class="flex items-center justify-between mb-4">
                ...
                @if($isReadOnly)
                    <span class="text-xs bg-green-100 text-green-700 font-semibold px-2 py-1 rounded-full">Fully Paid</span>
                @endif
            </div>
            ...
            @if($isReadOnly)
                <div class="mb-4 px-4 py-3 rounded-lg bg-green-50 border border-green-200 text-green-800 text-sm flex items-center gap-2">
                    ...
                    This receipt has been fully paid. No further edits allowed.
                </div>
            @endif
            ...
            <form method="POST"
                  action="{{ route('super_admin.br-receipt-payments.upsert', $receipt->id) }}"
                  class="space-y-4"
                  id="paymentForm">
                @csrf
                @method('PUT')
                ...
                <input type="hidden" name="br_receipt_id" value="{{ $receipt->id }}">
                <input type="hidden" name="total_amount" value="{{ $receipt->grand_total }}">

                {{-- Client Name --}}
                <div>
                    <label class="block text-xs font-medium text-gray-500 mb-1">
                        Client Name <span class="text-[#FF5757]">*</span>
                    </label>
                    <input type="text" name="client_name"
                           {{ $isReadOnly ? 'readonly' : 'required' }}
                           value="{{ old('client_name', $payment?->client_name ?? $receipt->delivered_to) }}"
                           placeholder="e.g. Sta. Ignacia Petron" readonly
                           class="w-full px-3 py-2 border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#FF5757]
                                  ...
                                  @error('client_name') border-red-400 @enderror">
                ...
                <div>
                    ...
                    <input type="number" name="down_payment" step="0.01" min="0"
                           ...
                           value="{{ old('down_payment', $payment?->down_payment ?? $receipt->downpayment) }}"
                           placeholder="0.00"
                           class="w-full px-3 py-2 border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#FF5757]
                                  {{ $isReadOnly ? 'bg-gray-50 border-gray-200 text-gray-500 cursor-not-allowed' : 'border-gray-300' }}
                                  @error('down_payment') border-red-400 @enderror">
                    @error('down_payment') <p class="text-xs text-red-500 mt-1">{{ $message }}</p> @enderror
                </div>
                <div>
                    <label class="block text-xs font-medium text-gray-500 mb-1">Down Payment Date</label>
                    <input type="date" name="down_payment_date"
                           {{ $isReadOnly ? 'readonly' : '' }} readonly
                           value="{{ old('down_payment_date', $payment?->down_payment_date?->format('Y-m-d') ?? $receipt->created_at->format('Y-m-d')) }}"
                           class="w-full px-3 py-2 border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#FF5757]
                                  {{ $isReadOnly ? 'bg-gray-50 border-gray-200 text-gray-500 cursor-not-allowed' : 'border-gray-300' }}">
                </div>
                ...
                <div>
                    ...
                    <textarea name="notes" rows="3"
                              ...
                              class="w-full px-3 py-2 border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#FF5757] resize-none
                                     {{ $isReadOnly ? 'bg-gray-50 border-gray-200 text-gray-500 cursor-not-allowed' : 'border-gray-300' }}">{{ old('notes', $payment?->notes) }}</textarea>
                </div>
                ...
                <div class="bg-gray-50 rounded-lg p-3 border border-gray-200 text-sm">
                    ...
                    <div class="flex justify-between text-gray-500 mb-1">
                        ...
                        <span class="font-mono text-green-600" id="liveAmountPaid">
                            ₱ {{ number_format($paid, 2) }}
                        </span>
                    </div>
                    <div class="flex justify-between font-bold border-t border-gray-200 pt-1 mt-1">
                        <span>Remaining Balance</span>
                        <span class="font-mono {{ $balance > 0 ? 'text-red-600' : 'text-green-600' }}" id="liveBalance">
                            {{ $balance > 0 ? '₱ ' . number_format($balance, 2) : 'Fully Paid ✓' }}
                        </span>
                    </div>
                </div>
